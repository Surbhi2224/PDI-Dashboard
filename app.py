...
# ============================================
    ...
    # Aggregate & top 10 issues
    top_issues = df.groupby("Issue Type")["Count"].sum().reset_index()
    top_issues = top_issues.sort_values(by="Count", ascending=False).head(10)

    st.metric("Top 10 Issues Total", int(top_issues["Count"].sum()))

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=top_issues["Issue Type"],
        y=top_issues["Count"],
        text=top_issues["Count"],
        textposition='outside'
    ))
    fig.update_layout(title=f"Top 10 Issues - {page}", xaxis_title="Issue Type", yaxis_title="Count")
    st.plotly_chart(fig)

    # Pareto chart
    top_issues["Cum%"] = top_issues["Count"].cumsum() / top_issues["Count"].sum() * 100
    figp = go.Figure()
    ...
    figp.update_layout(
        title=f"Top 10 Issues Pareto - {page}",
